<template>
  <main class="pb-40">
    <page-heading title="Edición usuario" :back-route="`/users/${$route.params.id}`" :breadcrumbs="breadcrumbs" />
    <form-section>
...
export default class NewUserPage extends Vue {
  ...
  @Users.State('user') currentUser!: User;
  ...
  setCurrentUser() {
    ...
    this.form.subjectId = this.currentUser?.subject?.id ?? 0;
    this.form.roleId = this.currentUser?.role?.id ?? 0;
