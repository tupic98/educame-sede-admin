<template>
  <main class="pb-40">
    <page-heading title="Nueva materia" back-route="/subject" :breadcrumbs="breadcrumbs" />
    <form-section>
...
export default class NewSubject extends Vue {
  ...
  @SubjectModel.Action('store') createSubject!: ({ subject, vm }: { subject: any; vm: any }) => ActionMethod;
  ...
  async onSubmit() {
    ...
    if (isValid) {
      try {
        await this.createSubject({ subject: this.form, vm: this });
