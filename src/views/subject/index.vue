  ...
  <main class="pb-40">
    ...
    >
      <template slot="actions">
        <custom-button title="Nueva materia" color="secondary" to="/subject/new">
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
        ...
        <template slot="items">
          ...
          >
            ...
            <td
              class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap"
            >
              {{ subject && subject.grade ? subject.grade.grade : '' || '' }}
            </td>
            <td
...
export default class SubjectPage extends Vue {
  ...
  breadcrumbs: Breadcrumb[] = [{ name: 'Administración' }, { name: 'Materia' }]
  ...
  selectedItem = 0
  @SubjectModel.State('subjects') subjects!: Subject[];
  @SubjectModel.State('meta') meta!: Meta;
  @SubjectModel.State('isLoading') isSubjectLoading!: boolean;
  @SubjectModel.Action('fetch') fetchSubjects!: ({ filters, vm }: { filters: Filters; vm: any }) => AxiosResponse;

  async mounted() {
    await this.fetchSubjects({ filters: this.filters, vm: this });
  }
