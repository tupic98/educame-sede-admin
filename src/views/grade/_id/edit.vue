<template>
  <main class="pb-40">
    <page-heading title="Nuevo grado" :back-route="`/grade/${$route.params.id}`" :breadcrumbs="breadcrumbs" />
    <form-section>
...
export default class EditGradePage extends Vue {
  ...
  @GradeModel.Action('update') updateGrade!: ({ grade, vm }: { grade: any; vm: any }) => ActionMethod;
  @GradeModel.Action('show') fetchGrade!: ({ id, vm }: { id: number; vm: any }) => ActionMethod;

  async mounted() {
    try {
      await this.fetchGrade({ id: +this.$route.params.id, vm: this });
  ...
  async onSubmit() {
    ...
    if (isValid) {
      try {
        await this.updateGrade({ grade: this.form, vm: this });
