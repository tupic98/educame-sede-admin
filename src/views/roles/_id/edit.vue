<template>
  <main class="pb-40">
    <page-heading title="Edición rol" :back-route="`/roles/${$route.params.id}`" :breadcrumbs="breadcrumbs" />
    <form-section>
...
export default class EditRolePage extends Vue {
  ...
  @RoleModel.State('role') currentRole!: Role;
  ...
  async mounted() {
    try {
      await this.fetchRole({ id: +this.$route.params.id, vm: this });
  ...
  setCurrentRole() {
    const { permissions } = this.currentRole;
    this.$set(this.form, 'id', this.currentRole.id);
    this.$set(this.form, 'name', this.currentRole.name);
    this.$set(this.form, 'type', this.currentRole.type);
    this.newPermissions = permissions.map((permission: Permission) => permission.id);
  }
  ...
  async onSubmit() {
    ...
    if (isValid) {
      try {
        ...
        await this.updateRole({ role: this.form, vm: this });
